...
void serialize_packet(const protocol::Packet& packet, uint8_t* buffer) {
    ...
    // group_id
    for (int i = 0; i < 8; i++) {
        buffer[i + 1] = static_cast<uint8_t>(packet.group_id >> ((7 - i) * 8));
    }

    // device_id
    for (int i = 0; i < 8; i++) {
        buffer[i + 9] = static_cast<uint8_t>(packet.device_id >> ((7 - i) * 8));
    }

    // message_id
    for (int i = 0; i < 4; i++) {
        buffer[i + 17] = static_cast<uint8_t>(packet.message_id >> ((3 - i) * 8));
    }
...
void deserialize_packet(const uint8_t* buffer, protocol::Packet& packet) {
    ...
    packet.group_id = 0;
    for (int i = 0; i < 8; i++) {
        packet.group_id = (packet.group_id << 8 | buffer[i + 1]);
    }

    packet.device_id = 0;
    for (int i = 0; i < 8; i++) {
        packet.device_id =
            (packet.device_id << 8) | buffer[i + 9];
    }

    packet.message_id = 0;
    for (int i = 0; i < 4; i++) {
        packet.message_id =
            (packet.message_id << 8) | buffer[i + 17];
    }
